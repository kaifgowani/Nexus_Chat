...
import { Shield, Hash, X, Plus, Lock, LogOut } from 'lucide-react';
...
export default function LeftSidebar({
  ...
  const publicChannels = groups.filter(g => g.type === 'channel' && (!g.isPrivate || g.members?.includes(currentUserId)));
  const directMessages = groups.filter(g => g.type === 'dm' && g.members?.includes(currentUserId));
  ...
  return (
    <div className={`${isSidebarOpen ? 'flex' : 'hidden'} md:flex flex-col w-64 bg-slate-950 border-r border-slate-800 absolute md:relative z-40 h-full`}>
      ...
      <div className="h-14 flex items-center px-4 border-b border-slate-800 justify-between">
        ...
        <button className="md:hidden text-slate-400 hover:text-white" onClick={onCloseSidebar}><X className="h-5 w-5" /></button>
      </div>
      ...
      <div className="flex-1 overflow-y-auto py-4 custom-scrollbar">
        ...
        <div className="mb-6">
          <div className="px-4 flex items-center justify-between group mb-2">
            ...
            <button onClick={onOpenCreateChannel} className="text-slate-400 hover:text-white opacity-0 group-hover:opacity-100 transition-opacity" title="New channel">
            ...
            {publicChannels.map(channel => (
              <li key={channel.id}>
                <button
                  ...
                  className={`w-full flex items-center gap-2 px-2 py-2 rounded-md transition-colors text-left text-sm ${currentGroupId === channel.id ? 'bg-indigo-600/10 text-indigo-400 font-medium' : 'text-slate-400 hover:bg-slate-800 hover:text-slate-200'}`}
                >
                ...
                <li key={dm.id}>
                  ...
                  >
                    <div className="relative flex-shrink-0">
                      <img src={otherUser.photoURL || `https://api.dicebear.com/7.x/avataaars/svg?seed=${otherUser.name}`} alt="" className="h-5 w-5 rounded-md bg-slate-800 object-cover" />
